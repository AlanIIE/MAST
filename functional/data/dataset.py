...
class CorrespondenceDataset(Dataset):
    ...
    def get_image(self, img_names, idx):
        ...
        image = self.l_prep(self.l_loader(img_name))
        return image
...
#         self.normalize = transforms.Normalize([50,0,0], [50,127,127])
...
#             # print('image shape before: ', image.shape)
#             if w%M != 0: sample[key] = sample[key][:,:,:-(w%M)]
#             if h%M != 0: sample[key] = sample[key][:,:,-(h%M)]
#             # print('image shape after: ', image.shape)
#         return sample
